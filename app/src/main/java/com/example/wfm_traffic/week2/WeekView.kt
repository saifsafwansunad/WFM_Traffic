...
import com.example.wfm_traffic.week2.WeekViewUtil.daysBetween
import com.example.wfm_traffic.week2.WeekViewUtil.getPassedMinutesInDay
import com.example.wfm_traffic.week2.WeekViewUtil.isSameDay
import com.example.wfm_traffic.week2.WeekViewUtil.today
import com.example.wfm_traffic.week2.WeekViewUtil
...
class WeekView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {
     ...
     */
    val weekViewLoader: WeekViewLoader = PrefetchingWeekViewLoader(MonthLoader(object :
    ...
    var monthChangeListener: MonthLoader.MonthChangeListener? = null
    ...
    private fun drawHeaderRowAndEvents(canvas: Canvas) {
        ...
        if (oldFirstVisibleDay == null || !WeekViewUtil.isSameDay(
                firstVisibleDay!!,
                oldFirstVisibleDay
            )
        ) {
        ...
        when {
            ...
            !TextUtils.isEmpty(sideTitleText) && TextUtils.isEmpty(sideSubtitleText) ->
                sideTitleText?.let { canvas.drawText(it, mHeaderColumnWidth / 2, (headerTitleAndSubtitleTextHeight + headerWeekDayTitleTextHeight) / 2.0f + weekDayHeaderRowPaddingTop, sideTitleTextPaint) }
            !TextUtils.isEmpty(sideTitleText) && !TextUtils.isEmpty(sideSubtitleText) -> {
                sideTitleText?.let { canvas.drawText(it, mHeaderColumnWidth / 2, headerWeekDayTitleTextHeight + weekDayHeaderRowPaddingTop, sideTitleTextPaint) }
                sideSubtitleText?.let { canvas.drawText(it, mHeaderColumnWidth / 2, headerTitleAndSubtitleTextHeight + weekDayHeaderRowPaddingTop, sideSubtitleTextPaint) }
            }
            TextUtils.isEmpty(sideTitleText) && !TextUtils.isEmpty(sideSubtitleText) ->
                sideSubtitleText?.let { canvas.drawText(it, mHeaderColumnWidth / 2, (headerTitleAndSubtitleTextHeight + sideSubtitleTextPaint.textSize) / 2.0f + weekDayHeaderRowPaddingTop, sideSubtitleTextPaint) }
        }
        ...
        if (containsAllDayEvent && !TextUtils.isEmpty(allDaySideTitleText)) {
            ...
            allDaySideTitleText?.let { canvas.drawText(it, mHeaderColumnWidth / 2, (top + bottom) / 2, allDaySideTitleTextPaint) }
            canvas.restore()
        }
    }
